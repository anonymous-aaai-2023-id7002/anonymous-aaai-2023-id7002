...
class PairwiseComparator(ArchNetwork, nn.Module):
    ...
    def compare(self, arch_1, arch_2):
        ...
        if self.pairing_method == "concat":
            ...
            score /= 2
            """
            emb_a = torch.cat((emb_1, emb_2), dim=-1)
            score_1 = torch.sigmoid(self.mlp(emb_a))
            emb_b = torch.cat((emb_2, emb_1), dim=-1)
            score_2 = torch.sigmoid(self.mlp(emb_b))
            score = (score_1 + 1 - score_2) / 2
    ...
    def _cal_compare_loss(self, arch_1, arch_2, better_labels, mlp) -> Tensor:
        emb_1 = self.arch_embedder(arch_1)
        emb_2 = self.arch_embedder(arch_2)
    ...
    def cal_compare_loss(self, arch_1, arch_2, better_labels) -> Tensor:
        return self._cal_compare_loss(arch_1, arch_2, better_labels, self.mlp)

    def update_compare(self, arch_1, arch_2, better_labels):
        pair_loss = self.cal_compare_loss(arch_1, arch_2, better_labels)
...
class DynamicEnsemblePairwiseComparator(PairwiseComparator):
    r"""
    Dynamic ensemble pairwise comparator.
    ...
    Args:
        ...
        use_uniform_confidence (bool): Whether use uniform confidence. Default: `False`.
    """
    NAME = "dynamic_ensemble_pairwise_comparator"
    ...
    def __init__(self, search_space, 
            ...
            compare_loss_type: str = "margin_linear",
            compare_margin: float = 0.01,
            pairing_method: str = "concat",
            diff_only: bool = False,
            train_use_sigmoid: bool = False,
            sorting_residue_worse_thresh: int = 100,
            sorting_residue_better_thresh: int = 100,
            max_grad_norm: float = None,
            schedule_cfg: Optional = None) -> None:
        # [optional] arch reconstruction loss (arch_decoder_type/cfg)
        
        super(DynamicEnsemblePairwiseComparator, self).__init__(
            search_space, arch_embedder_type, arch_embedder_cfg,
            mlp_hiddens, mlp_dropout,
            optimizer, scheduler,
            compare_loss_type, compare_margin,
            pairing_method, diff_only, 
            train_use_sigmoid,
            sorting_residue_worse_thresh,
            sorting_residue_better_thresh,
            max_grad_norm,
            schedule_cfg
        )
        ...
        if self.use_uniform_confidence:
            ...
            self.confidence_mlp = self.construct_mlp(
                2 * dim, mlp_hiddens, mlp_dropout, auxiliary_head_num)
    ...
    def mtl_update_compare(self, auxiliary_datas, margin = None):
        loss = 0.
        for model, (arch_1, arch_2, better_lst) in zip(self.module_lst, auxiliary_datas):
            loss += model.cal_compare_loss(arch_1, arch_2, better_lst)
        return self.update_step(loss)

    def update_compare(self, arch_1, arch_2, better_labels) -> float:
        score = self.compare(arch_1, arch_2)
        loss = self._get_loss(score, better_labels)
        return self.update_step(loss)

    def mtl_compare(self, arch_1, arch_2) -> Tuple[Tensor, List[Tensor]]:
        score = self.compare(arch_1, arch_2)
        lf_score_lst = [model.compare(arch_1, arch_2) for model in self.module_lst]
        return score, lf_score_lst

    def weighted_score(self, arch_1, arch_2) -> Tensor:
        score_lst = self.separate_score_compare(arch_1, arch_2)
        confidence_ratio = self._confidence_ratio(arch_1, arch_2)
        score = (confidence_ratio * score_lst)
        return score

    def separate_score_compare(self, arch_1, arch_2) -> Tensor:
        score_lst = torch.cat([
            model.compare(arch_1, arch_2).unsqueeze(1)
            for model in self.module_lst], 1
        )
        return score_lst

    def compare(self, arch_1, arch_2) -> Tensor:
        weighted_score = self.weighted_score(arch_1, arch_2)
        score = torch.sigmoid(weighted_score.sum(1))
        return score

    def _confidence_ratio(self, arch_1, arch_2) -> Tensor:
        if self.use_uniform_confidence:
            confidence_ratio = torch.softmax(self.confidence, 1)
        else:
            emb_1 = self.arch_embedder(arch_1)
            emb_2 = self.arch_embedder(arch_2)
            emb_a = torch.cat((emb_1, emb_2), dim = -1)
            confidence_1 = torch.sigmoid(self.confidence_mlp(emb_a))
            emb_b = torch.cat((emb_2, emb_1), dim = -1)
            confidence = (confidence_1 + torch.sigmoid(self.confidence_mlp(emb_b))) / 2
            confidence_ratio = torch.softmax(confidence, 1)
        return confidence_ratio
